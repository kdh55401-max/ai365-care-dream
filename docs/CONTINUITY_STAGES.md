...
- eview_status(승인/반려): 기록 검토로 그대로 유지. 새 판단·안전 검토와 별개.
- manager_status·ctual_followup_type: 연구용 2단계 평가 폼의 값으로만 남기고 **새 업무 상태로 해석하거나 옮기지 않았다**.
- 공개 메모(eview_note_visible_to_caregiver): 반려 사유·검토 메모 공개 기능으로 그대로. 조치·요청·응답 이력을 대신하지 않는다.
- PriorityCareStrip: 브라우저가 최신 500건으로 세던 숫자 4개를 **서버 전체 집계 카드 4개**로 바꿨다(아래).
- 최초 제출·검토 시각: 기존에는 불변 기록이 없었다 → eport_events 트리거로 **마이그레이션 적용 시점부터** 남긴다. 과거는 채우지 않는다.
...
| 테이블 | 성격 | 핵심 |
|---|---|---|
| eport_events | 불변(수정·삭제 트리거 차단) | eports 트리거가 제출 1회·승인/반려마다 기록. 행위자 범위(요양보호사 세션/기관 공유 관리자)·요청 id |
| dmin_decisions | 불변, 새 판단이 이전 판단을 대체(삭제 안 함) | 추가 조치 불필요 / 추가 관찰 필요 / 조치 필요 / 판단 보류 · 근거 · 이전 판단 id |
...
| ction_obligations | 현재 기한 + **바뀌지 않는 최초 기한** | 현장 응답기한 / 관리자 결과 재확인기한 / 관리자 직접 수행기한 · 종류(특정 일시/다음 실제 방문/미정) · 상태(미활성/미게시/진행 중/이행/취소) · 주기 번호 |
| ction_events | 불변, 요청 하나 = 한 줄(request_id 유일) | 생성·전환·수정·기한 변경(이전/새/최초 기한, 당시 연체 여부)·완료·취소·재개 · 이유 · 당시 담당 |
...
- 기관 첫 화면: 카드 4개(안전 신호 미검토·새 보고 미확인·기한 지난 조치·오늘 재확인 — 단위가 달라 합산하지 않음, 기준 시각 표시)
  → 누르면 /admin/work/:card에서 같은 계산의 전체 목록. 아래 "지금 할 일"은 운영 규칙 순서 통합 목록(임상 중증도 아님).
